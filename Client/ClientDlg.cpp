...
BOOL CClientDlg::OnInitDialog()
{
	...
	WSAAsyncSelect(m_Socket,m_hWnd,CM_RECEIVED,FD_READ | FD_ACCEPT);
	m_X = m_Y = 0;
...
void CClientDlg::OnOK() 
{
	...
	int width  = /*300*/500;									//获取屏幕的宽度
	int height = /*300*/500;									//获取屏幕的高度
	...
	m_cursorX = cursorpos.x - m_RecvX;
	m_cursorY = cursorpos.y - m_RecvY ;
	...
	TRACE("width=%d,height=%d,count=%d,mod=%d",width,height,count,mod);
	if ( mod != 0)
		count+=1;
...
LRESULT CClientDlg::OnReceived(WPARAM wParam,LPARAM lParam)
{
...
int CClientDlg::SendData(UINT index, int mod , int bmpsize , int frames,char *pSendBuf, sockaddr_in &addr)
{
	...
	if ( mod ==0 || index != frames-1)
		packsize = 2+2+bmpsize+4+2+2+4+2+2;
	else 
		packsize = 2+2+mod+4+2+2+4 +2+2;
	...
	*(WORD*)&pPackage[0] = index;		//填充序号
	...
	memcpy(&pPackage[4],pSendBuf,packsize-20);

	//填充位图大小
	*(int*)&pPackage[packsize-16]= bmpsize;
	//填充屏幕X坐标
	*(WORD*)&pPackage[packsize-12]= m_X;
	//填充屏幕Y坐标
	*(WORD*)&pPackage[packsize-10]= m_Y;
	//填充数据报大小
	*(int*)&pPackage[packsize-8] = packsize;
	*(WORD*)&pPackage[packsize-4] = m_cursorX;
	*(WORD*)&pPackage[packsize-2] = m_cursorY;
